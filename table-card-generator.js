...
class TableCardGenerator {
    ...
    // Returns full standalone HTML document for a movement card (used by generator)
    generateHTML(movement) {
        const fragment = movement.type === 'mitchell'
            ? this._buildMitchellHTML(movement)
            : this._buildTableCardsHTML(movement);
        const title = movement.type === 'mitchell'
            ? `Mitchell ${movement.pairs} Pairs Instructions`
            : `Table Cards ${movement.pairs} Pairs`;
        const css = `
            * { margin: 0; padding: 0; box-sizing: border-box; }
            body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Arial, sans-serif; background: white; padding: 8mm; }
            .cards-container { display: flex; flex-wrap: wrap; gap: 5mm; }
            .card-page { display: flex; flex-direction: row; gap: 5mm; align-items: flex-start; padding: 0 0 8mm 0; page-break-after: always; break-after: page; }
            ...
            .ns-box { border-color: #27ae60; } .ns-box h2 { color: #27ae60; }
            .ew-box { border-color: #e74c3c; } .ew-box h2 { color: #e74c3c; }
            ...
            .skip-notice { margin-top: 15px; padding: 14px 16px; background: #fff3cd; border: 2px solid #ffc107; border-radius: 10px; font-size: 14px; line-height: 1.6; color: #664d03; print-color-adjust: exact; -webkit-print-color-adjust: exact; }
            .sitout-mitchell-note { margin-bottom: 15px; padding: 12px 16px; background: #e8f4fd; border: 2px solid #3498db; border-radius: 10px; font-size: 13px; color: #1a5276; print-color-adjust: exact; -webkit-print-color-adjust: exact; }
            .mitchell-header { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; border-radius: 12px; padding: 20px; text-align: center; margin-bottom: 20px; print-color-adjust: exact; -webkit-print-color-adjust: exact; }
        ...
        `;
        return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>${title}</title><style>${css}</style></head><body>${fragment}</body></html>`;
    }
    ...
    _buildMitchellHTML(movement) {
        ...
        // Build per-round board assignment table
        const totalRounds = Math.max(...movement.movement.map(e => e.round));
        const numTables = movement.tables;
        let boardTableHTML = `
        <div style="margin-top:15px;">
            <h3 style="font-size:14px;font-weight:700;color:#2c3e50;margin-bottom:8px;">Board Assignments by Round</h3>
            <table style="width:100%;border-collapse:collapse;font-size:12px;">
                <thead><tr>
                    <th style="background:#34495e;color:white;padding:6px 4px;border:1px solid #bdc3c7;text-align:center;">Round</th>`;
        for (let t = 1; t <= numTables; t++) {
            boardTableHTML += `<th style="background:#34495e;color:white;padding:6px 4px;border:1px solid #bdc3c7;text-align:center;">T${t}</th>`;
        }
        boardTableHTML += `</tr></thead><tbody>`;
        for (let r = 1; r <= totalRounds; r++) {
            const isSkip = movement.skipRound && r === movement.skipRound;
            const rowStyle = isSkip ? 'background:#fff3cd;' : (r % 2 === 0 ? 'background:#f9f9f9;' : '');
            boardTableHTML += `<tr style="${rowStyle}"><td style="padding:5px 4px;border:1px solid #bdc3c7;text-align:center;font-weight:700;color:#2c3e50;">${r}${isSkip ? '⚠️' : ''}</td>`;
            for (let t = 1; t <= numTables; t++) {
                const entry = movement.movement.find(e => e.round === r && e.table === t);
                const boards = entry && entry.boards && entry.boards.length > 0
                    ? (entry.boards.length === 1 ? `${entry.boards[0]}` : `${entry.boards[0]}-${entry.boards[entry.boards.length-1]}`)
                    : '—';
                boardTableHTML += `<td style="padding:5px 4px;border:1px solid #bdc3c7;text-align:center;">${boards}</td>`;
            }
            boardTableHTML += `</tr>`;
        }
        boardTableHTML += `</tbody></table>
            ${movement.skipRound ? `<p style="font-size:10px;color:#664d03;margin-top:4px;">⚠️ = Skip round: EW move up TWO tables instead of one</p>` : ''}
        </div>`;
