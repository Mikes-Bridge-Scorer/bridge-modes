...
class RubberBridge extends BaseBridgeMode {
    ...
    handleHonorBonusInput(value) {
        if (value === 'BACK') {
            ...
            if (this.currentContract.suit === 'NT') {
                this.awardHonorBonus(150); // 4 Aces in NT
            } else {
                this.awardHonorBonus(100); // 4 Honors in suit
            }
            this.rubberState.honorBonusPending = false;
        } else if (value === 'Down') {
            this.awardHonorBonus(150); // 5 Honors (suits only)
            this.rubberState.honorBonusPending = false;
        }
        this.updateDisplay();
    }
    ...
    awardHonorBonus(bonus) {
        if (this.rubberState.lastContractSide && bonus > 0) {
            ...
            console.log(`🏅 Awarded ${bonus} honor points to ${this.rubberState.lastContractSide}`);
        }
    }
    ...
    getDisplayContent() {
        ...
        if (this.rubberState.rubberComplete) {
            ...
            return `
                ...
                <div class="game-content">
                    <div style="text-align: center; color: #f1c40f; font-size: 16px; margin: 8px 0;">🏆 RUBBER COMPLETE! 🏆</div>
                    <div style="text-align: center; font-size: 14px; margin: 6px 0;"><strong>${winner} wins ${gamesWon}-${gamesLost}</strong></div>
                    <div style="text-align: center; margin: 8px 0;">Final Score: NS ${totals.NS}, EW ${totals.EW}</div>
                </div>
                <div class="current-state">Press DEAL for New Rubber</div>
            `;
        }
        ...
        if (this.rubberState.honorBonusPending) {
            ...
            return `
                ...
                <div class="game-content">
                    ...
                    <div style="text-align: center; color: #f39c12; margin: 10px 0;">Honor bonuses for ${lastSide}?</div>
                    <div style="font-size: 12px; text-align: center; color: #bdc3c7;">${options}</div>
                </div>
        ...
        switch (this.inputState) {
            case 'level_selection':
                return baseDisplay + '</div><div class="current-state">Select bid level (1-7)</div>';
            case 'suit_selection':
                return baseDisplay + `<div><strong>Level: ${this.currentContract.level}</strong></div></div><div class="current-state">Select suit</div>`;
            case 'declarer_selection':
                ...
                return baseDisplay + `<div><strong>Contract: ${contractSoFar}${doubleText}</strong></div></div><div class="current-state">${this.currentContract.declarer ? 'Made/Plus/Down or X for double' : 'Select declarer (N/S/E/W)'}</div>`;
            case 'result_type_selection':
                const contract = `${this.currentContract.level}${this.currentContract.suit}${this.currentContract.doubled}`;
                return baseDisplay + `<div><strong>${contract} by ${this.currentContract.declarer}</strong></div></div><div class="current-state">Made exactly, Plus overtricks, or Down?</div>`;
            case 'result_number_selection':
                ...
                return baseDisplay + `<div><strong>${fullContract} by ${this.currentContract.declarer}</strong></div></div><div class="current-state">Enter number of ${modeText}</div>`;
            case 'scoring':
                return baseDisplay + `<div><strong>Deal completed</strong></div></div><div class="current-state">Press <strong>Honors</strong> button to claim bonuses, or <strong>Deal</strong> for next hand</div>`;
        }
    ...
    getHelpContent() {
        return {
            ...
            content: 'Traditional rubber bridge scoring. First to win 2 games wins the rubber. Use the Honors button to claim honor bonuses after scoring.',
