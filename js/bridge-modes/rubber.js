...
class RubberBridge extends BaseBridgeMode {
    ...
    getDisplayContent() {
        ...
        if (this.inputState === 'level_selection') {
            ...
            if (this.resultMode === 'down') {
                const maxDown = Math.min(7, 6 + this.currentContract.level);
                return baseDisplay + '<div><strong>' + fullContract + ' by ' + this.currentContract.declarer + '</strong></div></div><div class="current-state">Tricks down (1-' + maxDown + ')</div>';
            } else {
                const maxOvertricks = 13 - (6 + this.currentContract.level);
                return baseDisplay + '<div><strong>' + fullContract + ' by ' + this.currentContract.declarer + '</strong></div></div><div class="current-state">Overtricks (1-' + maxOvertricks + ')</div>';
            }
        } else if (this.inputState === 'scoring') {
    ...
    getHelpContent() {
        return {
            ...
            content: 'Traditional rubber bridge scoring. First to win 2 games wins the rubber. Games are won by reaching 100+ points below the line. Vulnerability affects bonuses and penalties. Use the Honors button to claim honor bonuses after scoring.',
