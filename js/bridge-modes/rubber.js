...
class RubberBridge extends BaseBridgeMode {
    ...
    calculateScore() {
        ...
        return {
            ...
            contractMade: result === '=' || (result && result.startsWith('+'))
        };
    }
    
    calculateAndRecordScore() {
        const scoreResult = this.calculateScore();
        const belowLineScore = scoreResult.belowLineScore;
        const aboveLineScore = scoreResult.aboveLineScore;
        const declarerSide = scoreResult.declarerSide;
        const contractMade = scoreResult.contractMade;
        if (contractMade) {
    ...
    getCurrentVulnerabilityString() {
        ...
        return '<div class="rubber-score-table" style="font-family: monospace; font-size: 11px; border: 1px solid rgba(255,255,255,0.3); margin: 4px 0;"><div style="display: flex; background: rgba(255,255,255,0.1);"><div style="flex: 1; text-align: center; padding: 2px; border-right: 1px solid rgba(255,255,255,0.3); font-size: 10px;"><strong>NS ' + (nsVuln ? '(V)' : '') + '</strong></div><div style="flex: 1; text-align: center; padding: 2px; font-size: 10px;"><strong>EW ' + (ewVuln ? '(V)' : '') + '</strong></div></div><div style="display: flex; min-height: 20px;"><div style="flex: 1; text-align: center; padding: 2px; border-right: 1px solid rgba(255,255,255,0.3); color: #3498db; font-size: 11px;">' + (nsAbove || '') + '</div><div style="flex: 1; text-align: center; padding: 2px; color: #3498db; font-size: 11px;">' + (ewAbove || '') + '</div></div><div style="border-top: 2px solid rgba(255,255,255,0.5); display: flex;"><div style="flex: 1; text-align: center; padding: 2px; border-right: 1px solid rgba(255,255,255,0.3); color: #e74c3c; font-weight: bold; font-size: 11px;">' + (nsBelow || '') + '</div><div style="flex: 1; text-align: center; padding: 2px; color: #e74c3c; font-weight: bold; font-size: 11px;">' + (ewBelow || '') + '</div></div><div style="display: flex; background: rgba(255,255,255,0.05); font-size: 9px;"><div style="flex: 1; text-align: center; padding: 1px; border-right: 1px solid rgba(255,255,255,0.3);">' + '★'.repeat(nsGames) + '☆'.repeat(2-nsGames) + '</div><div style="flex: 1; text-align: center; padding: 1px;">' + '★'.repeat(ewGames) + '☆'.repeat(2-ewGames) + '</div></div></div>';
    }
    ...
    generateScorecard() {
        ...
        return '<div style="font-family: monospace; font-size: 11px; border: 1px solid #f1c40f; margin: 8px 0; background: rgba(241,196,15,0.1); max-width: 100%;"><div style="background: #f1c40f; color: #2c3e50; text-align: center; padding: 6px; font-weight: bold; font-size: 12px;">🏆 ' + winner + ' WINS RUBBER 🏆</div><div style="padding: 8px;"><div style="display: flex; margin-bottom: 8px;"><div style="flex: 1; text-align: center;"><div style="font-weight: bold; color: #2c3e50;">NS TOTAL</div><div style="font-size: 16px; font-weight: bold; color: #2c3e50;">' + nsTotal + '</div></div><div style="flex: 1; text-align: center;"><div style="font-weight: bold; color: #2c3e50;">EW TOTAL</div><div style="font-size: 16px; font-weight: bold; color: #2c3e50;">' + ewTotal + '</div></div></div><div style="text-align: center; border-top: 1px solid rgba(44,62,80,0.3); padding-top: 6px; font-size: 12px; color: #2c3e50;">Winning margin: <strong>' + margin + ' points</strong></div><div style="display: flex; margin-top: 6px; font-size: 10px; color: #7f8c8d;"><div style="flex: 1;">Above: ' + nsAbove + '<br>Below: ' + nsBelow + '</div><div style="flex: 1; text-align: right;">Above: ' + ewAbove + '<br>Below: ' + ewBelow + '</div></div></div></div>';
    }
    ...
    getHelpContent() {
        return {
            ...
            content: '<div class="help-section"><h4>What is Rubber Bridge?</h4><p><strong>Rubber Bridge</strong> is the traditional, classic form of bridge scoring. Two partnerships compete to be first to win 2 games, which completes the "rubber."</p></div><div class="help-section"><h4>Key Concepts</h4><ul><li><strong>Game:</strong> First partnership to score 100+ points "below the line"</li><li><strong>Rubber:</strong> First partnership to win 2 games</li><li><strong>Vulnerability:</strong> Winners of first game become vulnerable</li><li><strong>Two-Line Scoring:</strong> Contract points below, bonuses above</li><li><strong>Honor Bonuses:</strong> Extra points for holding trump honors</li></ul></div><div class="help-section"><h4>Honor Bonuses</h4><ul><li><strong>4 trump honors</strong> (A,K,Q,J,10): 100 points</li><li><strong>5 trump honors</strong> (all): 150 points</li><li><strong>4 aces in NT:</strong> 150 points</li><li>Must be held by one player in the partnership</li><li>Claimed after each deal by pressing "Made" button</li></ul></div><div class="help-section"><h4>Button Controls</h4><ul><li><strong>Made button:</strong> Claims honor bonuses when scoring</li><li><strong>Plus/Down buttons:</strong> 4/5 trump honors when claiming</li><li><strong>NT button:</strong> 4 aces when claiming in NT</li><li><strong>Deal button:</strong> No honors / New rubber when complete</li></ul></div>',
            buttons: [
...
export default RubberBridge;
    ...
    getDisplayContent() {
        ...
        if (this.rubberState.rubberComplete) {
            ...
            return '<div class="title-score-row"><div class="mode-title">' + this.displayName + '</div><div class="score-display">NS: ' + totals.NS + '<br>EW: ' + totals.EW + '</div></div><div class="game-content"><div style="text-align: center; color: #f1c40f; font-size: 16px; margin: 8px 0;">🏆 RUBBER COMPLETE! 🏆</div><div style="text-align: center; font-size: 14px; margin: 6px 0;"><strong>' + winner + ' wins ' + gamesWon + '-' + gamesLost + '</strong></div>' + this.generateScorecard() + '</div><div class="current-state">Press DEAL button for New Rubber<br><div style="background: #27ae60; color: white; padding: 4px 8px; border-radius: 4px; display: inline-block; margin-top: 4px; font-weight: bold;">DEAL = NEW RUBBER</div></div>';
        }
        if (this.rubberState.honorBonusPending) {
            return '<div class="title-score-row"><div class="mode-title">' + this.displayName + '</div><div class="score-display">NS: ' + totals.NS + '<br>EW: ' + totals.EW + '</div></div><div class="game-content"><div><strong>' + dealInfo + '</strong></div><div class="rubber-scores">' + this.generateRubberScoreDisplay() + '</div></div><div class="current-state">Honor bonuses for ' + this.rubberState.lastContractSide + '?<br><small>' + (this.currentContract.suit === 'NT' ? '4 Aces (150)' : '4 Trump (100) or 5 Trump (150)') + '</small></div>';
        }
        switch (this.inputState) {
            case 'level_selection':
                return '<div class="title-score-row"><div class="mode-title">' + this.displayName + '</div><div class="score-display">NS: ' + totals.NS + '<br>EW: ' + totals.EW + '</div></div><div class="game-content"><div><strong>' + dealInfo + '</strong></div><div class="rubber-scores">' + this.generateRubberScoreDisplay() + '</div></div><div class="current-state">Select bid level (1-7)</div>';
            case 'suit_selection':
                return '<div class="title-score-row"><div class="mode-title">' + this.displayName + '</div><div class="score-display">NS: ' + totals.NS + '<br>EW: ' + totals.EW + '</div></div><div class="game-content"><div><strong>' + dealInfo + '</strong></div><div><strong>Level: ' + this.currentContract.level + '</strong></div><div class="rubber-scores">' + this.generateRubberScoreDisplay() + '</div></div><div class="current-state">Select suit</div>';
            case 'declarer_selection':
                ...
                return '<div class="title-score-row"><div class="mode-title">' + this.displayName + '</div><div class="score-display">NS: ' + totals.NS + '<br>EW: ' + totals.EW + '</div></div><div class="game-content"><div><strong>' + dealInfo + '</strong></div><div><strong>Contract: ' + contractSoFar + doubleText + '</strong></div><div class="rubber-scores">' + this.generateRubberScoreDisplay() + '</div></div><div class="current-state">' + (this.currentContract.declarer ? 'Made/Plus/Down or X for double' : 'Select declarer (N/S/E/W)') + '</div>';
            case 'result_type_selection':
                const contract = this.currentContract.level + this.currentContract.suit + this.currentContract.doubled;
                return '<div class="title-score-row"><div class="mode-title">' + this.displayName + '</div><div class="score-display">NS: ' + totals.NS + '<br>EW: ' + totals.EW + '</div></div><div class="game-content"><div><strong>' + dealInfo + '</strong></div><div><strong>' + contract + ' by ' + this.currentContract.declarer + '</strong></div><div class="rubber-scores">' + this.generateRubberScoreDisplay() + '</div></div><div class="current-state">Made exactly, Plus overtricks, or Down?</div>';
            case 'result_number_selection':
                const fullContract = this.currentContract.level + this.currentContract.suit + this.currentContract.doubled;
                if (this.resultMode === 'down') {
                    const maxDown = Math.min(7, 6 + this.currentContract.level);
                    return '<div class="title-score-row"><div class="mode-title">' + this.displayName + '</div><div class="score-display">NS: ' + totals.NS + '<br>EW: ' + totals.EW + '</div></div><div class="game-content"><div><strong>' + dealInfo + '</strong></div><div><strong>' + fullContract + ' by ' + this.currentContract.declarer + '</strong></div><div class="rubber-scores">' + this.generateRubberScoreDisplay() + '</div></div><div class="current-state">Tricks down (1-' + maxDown + ')</div>';
                } else {
                    const maxOvertricks = 13 - (6 + this.currentContract.level);
                    return '<div class="title-score-row"><div class="mode-title">' + this.displayName + '</div><div class="score-display">NS: ' + totals.NS + '<br>EW: ' + totals.EW + '</div></div><div class="game-content"><div><strong>' + dealInfo + '</strong></div><div><strong>' + fullContract + ' by ' + this.currentContract.declarer + '</strong></div><div class="rubber-scores">' + this.generateRubberScoreDisplay() + '</div></div><div class="current-state">Overtricks (1-' + maxOvertricks + ')</div>';
                }
            case 'scoring':
                const lastEntry = this.gameState.getLastHistoryEntry();
                if (lastEntry) {
                    const contractDisplay = lastEntry.contract.level + lastEntry.contract.suit + lastEntry.contract.doubled;
                    return '<div class="title-score-row"><div class="mode-title">' + this.displayName + '</div><div class="score-display">NS: ' + totals.NS + '<br>EW: ' + totals.EW + '</div></div><div class="game-content"><div><strong>Deal ' + lastEntry.deal + ':</strong> ' + contractDisplay + ' by ' + lastEntry.contract.declarer + ' = ' + lastEntry.contract.result + '</div><div class="rubber-scores">' + this.generateRubberScoreDisplay() + '</div>' + (lastEntry.gameWon ? '<div style="color: #f1c40f; text-align: center; margin: 4px 0;">🎉 GAME to ' + lastEntry.scoringSide + '! 🎉</div>' : '') + '</div><div class="current-state">Press Made for honors, or Deal for next hand</div>';
                }
                break;
            default:
